/**
 * Pure TypeScript RFC 1321 MD5 & HMAC-MD5 implementation.
 * Zero external dependencies, client-side execution, fully typed.
 */
...
function coreMD5(words: number[], len: number): number[] {
  /* append padding */
...
/**
 * Computes MD5 hash from a Uint8Array input and returns raw digest bytes.
 */
export function computeMD5Digest(bytes: Uint8Array): Uint8Array {
  const words = bytesToWords(bytes);
  const bitLength = bytes.length * 8;
  const hashWords = coreMD5(words, bitLength);
  return wordsToBytes(hashWords);
}

/**
 * Computes HMAC-MD5 from secret key bytes and message bytes (RFC 2104).
 */
export function computeHmacMD5Digest(
  keyBytes: Uint8Array,
  messageBytes: Uint8Array
): Uint8Array {
  let key = keyBytes;
  if (key.length > 64) {
    key = computeMD5Digest(key);
  }
  ...
  const innerDigest = computeMD5Digest(innerData);

  const outerData = new Uint8Array(64 + 16);
  outerData.set(opad, 0);
  outerData.set(innerDigest, 64);
  return computeMD5Digest(outerData);
}
